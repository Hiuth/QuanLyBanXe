...
Connection* Check_TKKH = Check_TaiKhoanKhachHang->CheckDatabase();
NodeTaiKhoanKhachHang* N_TKKH;
TaiKhoanKhachHang* KH = new TaiKhoanKhachHang();
CheckDuLieu* DL_TKKH = new CheckDuLieu();
...
void TaiKhoanKhachHang::DangNhap() {
    ...
    if (DL_TKKH->CheckUserAccount(email, pass)) {
...
void TaiKhoanKhachHang::NhapDuLieuTaiKhoanKhachHang()
{
    ...
    for (int i = 0; i < n; i++) {
        ...
        if (DL_TKKH->CheckTKKH(Email)) {
            ...
            N_TKKH = new NodeTaiKhoanKhachHang(Email, TenTaiKhoan, MatKhau);
            KH->TaoTaiKhoanKhachHang(N_TKKH);
            cout << endl;
        }
    }
}
void TaiKhoanKhachHang::TaoTaiKhoanKhachHang(NodeTaiKhoanKhachHang* p)
{
    try {
        ...
        Statement* stmt;
        stmt = Check_TKKH->createStatement();
